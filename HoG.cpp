...
void HoG(double *pixels, double *params, int *img_size, double *dth_des, unsigned int grayscale){
    ...
    //extract the wideth and height of the image
    int img_width  = img_size[1];
    int img_height = img_size[0];
    //calculate the dimensions of the histograms based on image size and cell size
    int hist1= 2+ceil(-0.5 + img_height/cwidth);
    int hist2= 2+ceil(-0.5 + img_width/cwidth);
    //Calculates the size of each bin in the histogram based on the number of orientation bins and whether the orientation is defined in radians or degrees.
    ...
    int des_indx = 0;
    //define three-dimensional vectors histogram and block
    ...
    for(unsigned int y=0; y<img_height; y++) {
        for(unsigned int x=0; x<img_width; x++) {
            if (grayscale == 1){ 
                //calculating gradients in grayscale images
                ...
                //calculating gradients in color images
                if(x==0){
            ...
            /* experiment:
                        // Populate dth_des with both magnitude and orientation
            dth_des[des_indx] = grad_mag;
            dth_des[des_indx + 1] = grad_or;
            des_indx += 2; // Increment index for the next cell
            */
...
//plhs: array of pointers to mxArray objects--stand for Pointer to Left Hand Side
//
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    //dynamically allocating address of dth_des and its content
    plhs[0] = mxCreateDoubleMatrix((hist1-2-(block_size-1))*(hist2-2-(block_size-1))*nb_bins*block_size*block_size, 1, mxREAL);
    dth_des = mxGetPr(plhs[0]); //retrive a pointer to the real data of a MATLAB
